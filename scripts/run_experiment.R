...
s <- simulate_experiment(params = params, seed = 123,gradual_stress = TRUE)
...
m_cultivar <- lmer(Anet ~ treatment * week + (1 + treatment | cultivar) +
                    (1 | block) + (1 | plant_id), data = design)
...
m_ext <- extend(m, along = "plant_id", n = length(unique(design$plant_id)))
